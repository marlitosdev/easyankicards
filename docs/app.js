...
/* O ⚙ DO CABEÇALHO abre "Configurações" (dlgConfig) — tema, cor da
 * letra, idioma e o antigo rodapé técnico (chave da IA, base, ajuda,
 * backup, diagnóstico), tudo numa janela só, no computador e no
 * celular. Janela normal do app (mesmo abrirModal de sempre): fecha
 * sozinha em Esc/clique no fundo, sem precisar reinventar isso aqui. */
if ($("btnHdrConfig")) {
  $("btnHdrConfig").onclick = () => abrirModal("dlgConfig");
  attachTip($("btnHdrConfig"), "cfg_titulo");
}
if ($("btnConfigFechar")) $("btnConfigFechar").onclick = () => $("dlgConfig").close();
